  ...
  async onEnd(result: FullResult) {
    ...
    <head>
      ...
      <style>
        ...
        h1 {
          color: #F48031; /* Hex Accent Code Requirement */
        ...
        .card.passed h3 { color: #F48031; } /* Passing Indicator Status Hex */
      ...
      <div class="dashboard">
        ...
        ${this.testDetails.map(r => `
          <div class="test-item ${r.status === 'failed' ? 'failed' : ''}">
            <strong>${r.name}</strong> - <span style="color: ${r.status === 'passed' ? '#F48031' : '#ef4444'}">${r.status.toUpperCase()}</span> (${(r.duration / 1000).toFixed(2)}s)
    ...
    const reportPath = 'custom-report.html';
    try {
      const fs = require('fs');
      fs.writeFileSync(reportPath, htmlContent, 'utf-8');
      console.log(`\n✨ Glassmorphism HTML Report Generated successfully at: ${reportPath}`);
    } catch (e) {
      console.error('Failed to export HTML report file due to compiler restrictions:', e);
    }
  }
}
